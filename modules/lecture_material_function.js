...
module.exports = {
    select_lecture_materialfile: async (lecture_material_uniqueid) => {
        try{
            ...
            where lecture_material_id = ?`, [lecture_material_uniqueid])
            return result;
        }
        catch(err){
            throw err;
    ...
    select_lecture_material: async (lecture_material_uniqueid) =>{
        try{
            ...
            from lecture_material where id = ?`, [lecture_material_uniqueid])
            return result[0];
        }
        catch(err){
            throw err;
    ...
    select_lecture_materialid: async (name, semester, sub_code, lecture_materialid) =>{
        try{
            ...
            from lecture_material where professor_name = ? and semester = ? and sub_code = ?
            order by id limit ?,1`, [name, semester,sub_code,lecture_materialid])
